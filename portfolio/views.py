...
def result_page(request):
	...
	end_date = str((datetime.now() - timedelta(days=1)).date().isoformat())
	    ...
	    #print "1yes"
            ...
            for i in range(0,6):
		#print "1yes:"+str(i)
                curPrice=int(float(stckdict[item["sticker"]].get_price()))
                price_open = random.uniform(curPrice*95, curPrice*105)/100.0
                price_close = random.uniform(price_open*99, price_open *101)/100.0
		print "open " + str(price_open) + "close " + str(price_close)
                if len(day_pro)<=i:
		    if i==5: date_key="2017-05-10"
		    if i==4: date_key="2017-05-11"
		    if i==3: date_key="2017-05-12"
		    if i==2: date_key="2017-05-15"
		    if i==1: date_key="2017-05-16"
		    if i==0: date_key="2017-05-17"                    
		    day_pro.append({date_key:float(price_close)*int(float(item["aloc_amt"])/float(price_open))+float(item["aloc_amt"])%float(price_open)})
		    ...
		    if i==1: date_key="2017-05-16"
		    if i==0: date_key="2017-05-17" 
                    day_pro[i][date_key] = float(day_pro[i][date_key]) + float(price_close)*int(float(item["aloc_amt"])/float(price_open)) + float(item["aloc_amt"])%float(price_open)
